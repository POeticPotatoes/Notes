...
int split(ListNode *cur, ListNode **ptrEvenList,ListNode **ptrOddList)
{
    ListNode *a=cur, *b=cur->next, ta={0,NULL}, tb={0, NULL}, *x = &ta, *y = &tb;
    *ptrEvenList = x, *ptrOddList = y;
    while (b) {
        x->next = malloc(sizeof(ListNode));
        x = x->next, x->item = b->item;
        a = b->next;
        if (!a) break;
        y->next = malloc(sizeof(ListNode));
        y = y->next, y->item = a->item;
        b = a->next;
    }
    *ptrEvenList = (*ptrEvenList)->next, *ptrOddList= (*ptrOddList)->next;
}
