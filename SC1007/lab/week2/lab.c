...
ListNode *reverseList(LinkedList *ll, ListNode *p) {
    if (!p) return p;
    ListNode *c = reverseList(ll, p->next);
    if (!c) ll->head = p;
    else c->next = p;
    return p;
}
...
void recursiveReverse(Queue *q){
    ListNode *p = reverseList(&(q->ll), q->ll.head);
    p->next = NULL;
}
...
int palindrome(char *word){
    ...
    while (l<r) {
        ...
        while (word[r] == ' ') r--;
        if (l>=r) break;
        if (toupper(word[l]) != toupper(word[r])) {
            printf("String: %s\nVerdict: %s\n", word, "You suck!");
            return -1;
        }
        l++, r--;
...
int isOpener(char c) {
    for (int i=0;i<3;i++) if (opening[i] == c) return 1;
    return 0;
}
...
int balanced(char *expression){
    ...
    // if (!isEmptyStack(&S)) return 0; ?????? isEmptyStack doesn't work??
    if (lazy) return 0;
    return 1;
}
