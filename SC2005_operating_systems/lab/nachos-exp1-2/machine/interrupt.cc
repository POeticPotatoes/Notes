...
void Interrupt::Reschedule(int fromNow)
{
	PendingInterrupt * thing = (PendingInterrupt *) pending->SortedRemove(NULL);

    int when = stats->totalTicks + fromNow;
	thing->when = when;

    DEBUG('i', "Scheduling interrupt handler the %s at time = %d\n", 
					intTypeNames[thing->type], when);
    ASSERT(fromNow > 0);

    pending->SortedInsert(thing, when);
}
